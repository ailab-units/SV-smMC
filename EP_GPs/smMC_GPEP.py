...
class smMC_GPEP(object):
    ...
    def transform_data_w_pindex(self, data, property_index):
        x = data["params"]
        if property_index == -1:
            p = data["labels"]
        else:
            p = data["labels"][:,:,property_index]
    ...
    def compute_pac_bounds(self, x_train, y_train, x_test, y_test, epsilon = 0.05):
        n = x_train.shape[0]
        print('n = ', n)
        print('epsilon = ', epsilon)
        print(self.kernel)
        p_train = np.mean(y_train,axis=1)
        p_test = np.mean(y_test,axis=1)

        train_mean, train_variance = self.latentPrediction(x_train=x_train, x_test=x_train)
        train_pred_prob = self.getProbability(train_mean, train_variance)

        test_mean, test_variance = self.latentPrediction(x_train=x_train, x_test=x_test)
        test_pred_prob = self.getProbability(test_mean, test_variance)

        gen_err = np.mean(np.abs(p_test-test_pred_prob))
        print('gen_err = ', gen_err)

        emp_err = np.mean(np.abs(p_train-train_pred_prob))
        print('emp_err = ', emp_err)

        Kprior = RBF(0.0147)(x_train)
        Kprior = Kprior + self.CORRECTION * np.eye(len(Kprior))
        
        K = self.kernel(x_train)
        K = K+self.CORRECTION * np.eye(len(K))
        
        sigma_tilde = self.sigma_tilde

        inv_sigma_tilde = np.diag(1/np.diag(sigma_tilde))
        
        sigma = K-np.dot(K,np.dot(self.invC,K))
        mu = np.dot(sigma, np.dot(inv_sigma_tilde, self.mu_tilde))
        
        post_mean = torch.tensor(mu)
        post_cov = torch.tensor(sigma)

        prior_mean = torch.zeros_like(post_mean)
        prior_cov = torch.tensor(Kprior)

        prior_distrib = MultivariateNormal(loc=prior_mean, covariance_matrix=prior_cov)
        posterior_distr = MultivariateNormal(loc=post_mean, covariance_matrix=post_cov)
    
        kl_div = torch.distributions.kl_divergence(posterior_distr, prior_distrib)
        print('kl_div = ', kl_div)
    ...
    def avg_predictions(self, x_train, x_test):
        mean, variance = self.latentPrediction(x_train=x_train, x_test=x_test)
        post_mean = self.getProbability(mean, variance)
        return post_mean

    def avg_predictions_w_unc(self, x_train, x_test, z=1.96):
        mean, variance = self.latentPrediction(x_train=x_train, x_test=x_test)
        q1, q2 = self.getBounds(mean=mean, variance=variance, z=z, x_train=x_train)
        post_mean = self.getProbability(mean, variance)
    ...
    def fit(self, x_train, y_train, n_trajectories, max_iters=10000):
        ...
        print('optimal lengthscale = ', res.x)
        r = RBF(res.x)
        self.kernel = r
    ...
    def myLatentPrediction(self, x_train, x_test): # Rasmussen GPs p. 74
        
        ks = self.kernel(x_test, x_train)
        tmp = np.dot(ks, self.invC)
        fs = np.dot(tmp, self.mu_tilde)
        kss = self.kernel(x_test)
        vfs = kss - np.dot(tmp, ks.transpose())
        return fs, vfs

    def save(self, filepath, filename, idx=''):

        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        smc_dict = {"kernel":self.kernel, "invC":self.invC, "mu_tilde":self.mu_tilde, "sigma_tilde":self.sigma_tilde}

        with open(os.path.join(filepath, filename+idx+".pickle"), 'wb') as handle:
            pickle.dump(smc_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)

        file = open(os.path.join(filepath, filename+idx+"_training_time.txt"),"w")
        file.writelines(self.training_time)
        file.close()

    def load(self, filepath, filename, idx=''):

        with open(os.path.join(filepath, filename+idx+".pickle"), 'rb') as handle:
        ...
        self.sigma_tilde = smc_dict["sigma_tilde"]

        file = open(os.path.join(filepath, filename+idx+"_training_time.txt"),"r+")
