...
class GPmodel(ApproximateGP):
    ...
    def __init__(self, inducing_points, likelihood='binomial', variational_distribution='cholesky', 
        ...
        if len(inducing_points)>1000:
            rnd_idxs = np.random.randint(low=0, high=len(inducing_points)-1, size=1000)
            inducing_points = inducing_points[rnd_idxs]
    ...
    def compute_pac_bounds(self, train_data, test_data, pindex, device, epsilon = 0.05):
        ...
        self.to(device)
        x_train, y_train, n_train_samples, n_train_trials = get_bernoulli_data(train_data)
        x_test, y_test, n_test_samples, n_test_trials = get_bernoulli_data(test_data)
    ...
    def evaluate(self, train_data, pindex, n_posterior_samples, val_data=None, device="cpu"):
        ...
        x_train = get_bernoulli_data(train_data)[0]
